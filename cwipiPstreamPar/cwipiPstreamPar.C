...
int c2fconnec_size, int fconnec_size, int nbParts, float cwipiVerbose, double geom_tol)
{
    ...
    int appSuffix = floor((myGlobalRank - nMyProc)/nbParts) + 1;
...
interpolationCellPointWallModified<vector> triangulateCellsU, float cwipiVerbose, std::string globalPath, std::string UIntPath)
{
    ...
    int ens = floor((myGlobalRank - nMyProc)/nbParts) + 1;
...
void cwipiSend(const fvMesh& mesh, const volVectorField& vf, const Time& runTime, int cwipiIteration, int nbParts, float cwipiVerbose)
{
    ...
    label nMyProc=Pstream::myProcNo();
    MPI_Comm_rank(MPI_COMM_WORLD, &myGlobalRank);
    int appSuffix = floor((myGlobalRank - nMyProc)/nbParts) + 1;  // Normally works for any decomposition
    ...
    if (cwipiVerbose) Pout<< "Before sending to KF in member "<< appSuffix << " from processor " << Foam::Pstream::myProcNo() << endl;  
    ...
    switch(status)
    {
        ...
        if (cwipiVerbose) Pout << "Sent Ok in member " << appSuffix << " from processor " << Foam::Pstream::myProcNo() << endl << "\n";
        break;
...
void cwipiSendParams(const fvMesh& mesh, const volVectorField& vf, const Time& runTime, int cwipiIteration, int cwipiParams, int nbParts, float cwipiVerbose)
{
    ...
    if (cwipiVerbose == 1) Pout << "After affection if find ok " << endl;
    ...
    if (cwipiVerbose == 1) Pout << "After sending params to KF from member " << appSuffix << " from processor " << Foam::Pstream::myProcNo() << endl;
