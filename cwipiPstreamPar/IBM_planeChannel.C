...
double lambdaX, double lambdaY, double lambdaZ, double deltaChannel, int DarcyForchheimer, int nbParts, int partsReparty, float cwipiVerbose, std::string globalRootPath)
{
    ...
    alpha = static_cast<double*> (malloc(sizeof(double) * (Fourier_m + 1) * (Fourier_n + 1) * (Fourier_o + 1) * 3)); // 3 for the 3 components of the force
    beta = static_cast<double*> (malloc(sizeof(double) * (Fourier_m + 1) * (Fourier_n + 1) * Fourier_o * 3));
    gamma = static_cast<double*> (malloc(sizeof(double) * Fourier_m * (Fourier_n + 1) * (Fourier_o + 1) * 3));
    delta = static_cast<double*> (malloc(sizeof(double) * Fourier_m * (Fourier_n + 1) * Fourier_o * 3));
  
    if (DarcyForchheimer == 2){
        alpha = static_cast<double*> (realloc(alpha, sizeof(double) * (Fourier_m + 1) * (Fourier_n + 1) * (Fourier_o + 1) * 6));
        beta = static_cast<double*> (realloc(beta, sizeof(double) * (Fourier_m + 1) * (Fourier_n + 1) * Fourier_o * 6));
        gamma = static_cast<double*> (realloc(gamma, sizeof(double) * Fourier_m * (Fourier_n + 1) * (Fourier_o + 1) * 6));
        delta = static_cast<double*> (realloc(delta, sizeof(double) * Fourier_m * (Fourier_n + 1) * Fourier_o * 6));
    }
...
int nbParts, int partsReparty, float cwipiVerbose, std::string globalRootPath)
{
    ...
    double *DD = NULL;
    DD = static_cast<double*> (malloc(sizeof(double) * 9));
    std::ofstream myfile;
    myfile.open(UpdatedVariables, std::ios::out | std::ios::app);
