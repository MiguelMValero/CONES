...
void addControlParams(int numberCwipiPhase, int cwipiStep, double deltaT, double currentTime, int cwipiMembers, int cwipiObs, int cwipiParams, int nbParts, int partsReparty)
{
    ...
    cwipi_add_local_int_control_parameter("nbParts", nbParts);
    cwipi_add_local_int_control_parameter("partsReparty", partsReparty);
}
...
Foam::interpolationCellPointWallModified<Foam::vector> triangulateCellsU(U);
...
if (Foam::Pstream::master()){
	if (remove("/home/villanul/OpenFOAM/villanul-8/run/cwipi_tests/first_test/UInt") != 0)
		...
		Foam::Pout << "File successfully deleted with my processor " << Foam::Pstream::master() << Foam::nl << Foam::endl;
}
...
// Foam::Pout << "Processor number " << Foam::Pstream::myProcNo() << " is creating the file with interpolated velocities " <<
// Foam::nl << Foam::endl;
...
std::ifstream inFile(proj_file);
    if (inFile.is_open())
    {
        ...
        while( std::getline(inFile,line) )
        {
			...
			Foam::label ownCell = mesh.findCell(pointCoord);
			if (ownCell == -1){
				// Foam::Pout<< "Cell ID not found in my processor" << Foam::nl << Foam::endl;
				continue;
			}
			Foam::vector UatSP(Foam::vector::zero);
...
void cwipiSendParams(const fvMesh& mesh, const volVectorField& vf, const Time& runTime, int cwipiIteration, int cwipiParams, int nbParts, int partsReparty)
{
...
void cwipiRecvParams(const fvMesh& mesh, volVectorField& U, int cwipiParams, int nbParts, int partsReparty)
{
