...
int main(int argc, char *argv[])
{
 ...
 char RunCase[250]= "/";
  ...
  for (int i = firstCwipiPhase; i < numberCwipiPhase; i++)
  {
    ...
    MatrixXf UptMatrix = MatrixXf::Zero(nb_cells * 3 + cwipiParams, cwipiMembers);
    ...
    if (hyperlocSwitch == 1)
    {
        UptMatrix = KF_hyperloc(stateVector, obsMatrix, sampMatrix, cwipiMembers, nb_cells, cwipiObs, cwipiObsU, sigmaUserU, sigmaUserp, sigmaUserCf,
                                sigmaLocX, sigmaLocY, sigmaLocZ, localSwitch, clippingSwitch, hyperlocSwitch, cwipiParams, cwipiParamsObs, stateInfl, paramsInfl, typeInfl,
    ...
    else
    {
        ...
        UptMatrix = KF(stateVector, obsMatrix, sampMatrix, cwipiMembers, nb_cells, cwipiObs, cwipiObsU, sigmaUserU, sigmaUserp, sigmaUserCf,
                      sigmaLocX, sigmaLocY, sigmaLocZ, localSwitch, clippingSwitch, hyperlocSwitch, cwipiParams, cwipiParamsObs, stateInfl, paramsInfl, typeInfl,
                      typeInputs, velocityCase, sigmaUserUa, sigmaUserUb, sigmaUserUc, paramEstSwitch, mesh, cwipiVerbose, stringRootPath);
