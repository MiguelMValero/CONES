...
float clippingSwitch, float hyperlocSwitch, int nb_p, int cwipiParamsObs, double stateInfl, double paramsInfl, float typeInfl, int typeInputs, int velocityCase, double sigmaUserUa, double sigmaUserUb, double sigmaUserUc, float paramEstSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath);

Eigen::MatrixXf KF_hyperloc(const Eigen::Ref<const Eigen::MatrixXf>& velo_field_mat, const Eigen::Ref<const Eigen::ArrayXf>& obs_field, const Eigen::Ref<const Eigen::MatrixXf>& proj_field_mat, int nb_e, int nb_cells, int nb_o, int nb_oU, double sigma_userU, double sigma_userp, double sigma_userCf, double sigmaLocX, double sigmaLocY, double sigmaLocZ, float localSwitch, 
float clippingSwitch, float hyperlocSwitch, int nb_p, int cwipiParamsObs, double stateInfl, double paramsInfl, float typeInfl, int typeInputs, int velocityCase, double sigmaUserUa, double sigmaUserUb, double sigmaUserUc, float paramEstSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath);

void KF_output(double *sendValues, double *paramsSendValues, double *values, const Eigen::Ref<const Eigen::MatrixXf>& UptMatrix, const Eigen::Ref<const Eigen::MatrixXf>& sampMatrix, const Eigen::Ref<const Eigen::ArrayXf>& obsMatrix, int nb_e, int nb_cells, double time, int nb_p, int nb_oU, int nb_o, int cwipiParamsObs, int velocityCase, int index, int subdomains, double epsilon, float cwipiVerbose);
...
Eigen::MatrixXf localisation(const Eigen::Ref<const Eigen::MatrixXf>& KnoCorr, int nb_cells, int nb_p, int nb_o, int nb_oU, double sigmaLocX, double sigmaLocY, double sigmaLocZ, float clippingSwitch, float hyperlocSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath);
