...
float clippingSwitch, float hyperlocSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath)
{
...
float clippingSwitch, float hyperlocSwitch, int nb_p, int cwipiParamsObs, double stateInfl, double paramsInfl, float typeInfl, int typeInputs, int velocityCase, double sigmaUserUa, double sigmaUserUb, 
double sigmaUserUc, float paramEstSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath)
{
    ...
    if (localSwitch){
    K = localisation(K, nb_cells, nb_p, nb_o, nb_oU, sigmaLocX, sigmaLocY, sigmaLocZ, clippingSwitch, hyperlocSwitch, mesh, cwipiVerbose, stringRootPath);
    }
    else {
...
//===========================================================================
MatrixXf KF_hyperloc(const Eigen::Ref<const Eigen::MatrixXf>& velo_field_mat, const Eigen::Ref<const Eigen::ArrayXf>& obs_field, const Eigen::Ref<const Eigen::MatrixXf>& proj_field_mat, 
int nb_e, int nb_cells, int nb_o, int nb_oU, double sigma_userU, double sigma_userp, double sigma_userCf, double sigmaLocX, double sigmaLocY, double sigmaLocZ, float localSwitch, 
float clippingSwitch, float hyperlocSwitch, int nb_p, int cwipiParamsObs, double stateInfl, double paramsInfl, float typeInfl, int typeInputs, int velocityCase, double sigmaUserUa, double sigmaUserUb, 
double sigmaUserUc, float paramEstSwitch, const fvMesh& mesh, float cwipiVerbose, std::string stringRootPath)
{
    ...
    switch (velocityCase)
    {
        ...
        case 3:
            obs_hyperloc = 1;
            temp_obsMatrix = ArrayXf::Zero(obs_hyperloc); 
            temp_sampMatrix = MatrixXf::Zero(obs_hyperloc,nb_e);
    ...
    int count_clipsDone = 0;   // Count the number of clipping already done in the hyperlocalization
    int cellIndex = 0;     // Retrieve cell index in the clipping file for the loop

    MatrixXf velo_field_mat_upt = velo_field_mat;  // Save orginial state matrix to reconstruct it after hyperlocalization process

    int nb_clipCells = clippingCells(0);
    MatrixXf temp_state(nb_clipCells*3 + nb_p,nb_e);
    MatrixXf temp_state_upt;
    MatrixXf temp_params(nb_p,nb_e);
    ...
    for (int i=0; i<count_clippingCells; i++)
    {
        ...
        if (i==(nb_clipCells + count_clipsDone)){
            if (cwipiVerbose) std::cout << "Entered end of clipping if " << std::endl;
            if (cwipiVerbose) std::cout << "count_clipsDone value is = " << count_clipsDone << std::endl;
            //** Fill the parameters now that we have all the values**
            for (int j = 0; j < nb_e; j++){
                for (int k = 0; k < nb_p; k++){
                    temp_state(3*nb_clipCells + k, j) = velo_field_mat(3*nb_cells + k, j);
                }
            }
            ...
            switch (velocityCase)
            {
                ...
                case 2:
                case 3:
                    temp_obsMatrix(0) = obs_field(count_obs);
                    for (int j = 0; j < nb_e; j++){
                        temp_sampMatrix(0,j) = proj_field_mat(count_obs,j);
                    }
                    break;
                case 4:
                case 5:
                case 6:
                    if (cwipiVerbose) std::cout << "Inside case 6 " << temp_obsMatrix << std::endl;
                    temp_obsMatrix(0) = obs_field(count_obs);
                    temp_obsMatrix(1) = obs_field(nb_oU + count_obs);
                    if (cwipiVerbose) std::cout << "temp_obsMatrix filled = \n " << temp_obsMatrix << std::endl;

                    for (int j = 0; j < nb_e; j++){
                        temp_sampMatrix(0,j) = proj_field_mat(count_obs,j);
                        temp_sampMatrix(1,j) = proj_field_mat(nb_oU + count_obs,j);
                    }
                ...
                case 7:
                    temp_obsMatrix(0) = obs_field(count_obs);
                    temp_obsMatrix(1) = obs_field(nb_oU + count_obs);
                    temp_obsMatrix(2) = obs_field(2*nb_oU + count_obs);

                    for (int j = 0; j < nb_e; j++){
                        temp_sampMatrix(0,j) = proj_field_mat(count_obs,j);
                        temp_sampMatrix(1,j) = proj_field_mat(nb_oU + count_obs,j);
                        temp_sampMatrix(2,j) = proj_field_mat(2*nb_oU + count_obs,j);
                    }
                    break;
            }
            ...
            //** Perform EnKF on temp_state **
            //** Perform covariance localization on temp_state **
            //** Perform inflation on temp_sate **
            temp_state_upt = KF(temp_state, temp_obsMatrix, temp_sampMatrix, nb_e, nb_clipCells, obs_hyperloc, nb_oU, sigma_userU, sigma_userp, sigma_userCf,
                                        sigmaLocX, sigmaLocY, sigmaLocZ, localSwitch, clippingSwitch, hyperlocSwitch, nb_p, cwipiParamsObs, stateInfl, paramsInfl, typeInfl,
            ...
            if (cwipiVerbose) std::cout << "Params saved  " << std::endl;

            //** Update the values giving the update temp state **
            for (int j = 0; j < nb_e; j++){
                for (int k = nb_clipCells; k<nb_clipCells; k++){
                    cellIndex = clippingCells(k + count_clipsDone + 1);
                    velo_field_mat_upt(cellIndex, j) = temp_state_upt(k, j);
                    velo_field_mat_upt(nb_cells + cellIndex, j) = temp_state_upt(nb_clipCells + k, j);
                    velo_field_mat_upt(2*nb_cells + cellIndex, j) = temp_state_upt(2*nb_clipCells + k, j);
                }
            }
            if (i<(count_clippingCells-1)){                                   // if clippings are not done
                count_clipsDone = count_clipsDone + nb_clipCells +1;      // Update the count of the clipping already performed
                nb_clipCells = clippingCells(i+1);                        // Update the number of cells in next clipping
                count_obs = count_obs + 1;                                // New observation index
                temp_state.resize(nb_clipCells*3 + nb_p,nb_e);            // Resize temp_state over the new number of cells in the clipping
            }
            ...
            for (int j = 0; j < nb_e; j++){
                temp_state(i - count_clipsDone, j) = velo_field_mat(clippingCells(i+1), j);
                temp_state(nb_clipCells + i - count_clipsDone, j) = velo_field_mat(nb_cells + clippingCells(i+1), j);
                temp_state(2*nb_clipCells + i - count_clipsDone, j) = velo_field_mat(2*nb_cells + clippingCells(i+1), j);
            }
        }
    }

    if (cwipiVerbose) std::cout << "Hyperlocalization loop done, beginning to finalize the parameters  " << std::endl;
    temp_params = temp_params/nb_o;
    for (int j = 0; j < nb_e; j++){
        for (int k = 0; k < nb_p; k++){
            velo_field_mat_upt(3*nb_cells + k, j) = temp_params(k, j);
        }
    }
