...
int main(int argc, char *argv[])
{
  ...
  configValues = (double *)malloc(sizeof(double) * 35);
  ...
  for (int i = firstCwipiPhase; i < numberCwipiPhase; i++)
  {
    ...
    for (int j = 1; j < (cwipiMembers + 1); j++)
    {
	  ...
	  if (cwipiParams != 0) { // in case there are parameters to update with EnKF
    ...
    stateMatrixUpt = mainEnKF(stateMatrix, mesh, cwipiMembers, nb_cells, cwipiObs, cwipiObsU, sigmaUserU, sigmaUserp, sigmaUserCf, sigmaLocX, sigmaLocY, sigmaLocZ, localSwitch, clippingSwitch, hyperlocSwitch, cwipiParams, cwipiParamsObs, stateInfl, paramsInfl, typeInfl, typeInputs, velocityCase, sigmaUserUa, sigmaUserUb, sigmaUserUc, paramEstSwitch, stateEstSwitch,  cwipiVerbose, stringRootPath, cwipiTimedObs, obsTimeStep, time, init_time, epsilon);
