...
bool Foam::UPstream::init(int& argc, char**& argv, const bool needsThread)
{
    ...
    std::cout << "nMembers from UPstream.C = " << nMembers << std::endl;
    int appSuffix = floor((myGlobalRank + nbParts - 1)/nbParts);
    std::cout << "appSuffix from UPstream.C = " << appSuffix << std::endl;
...
)
{
    ...
    if (parentIndex == -1)
    {
        ...
        PstreamGlobals::MPICommunicators_[index] = localComm;
        MPI_Comm_group
        (
            localComm,
