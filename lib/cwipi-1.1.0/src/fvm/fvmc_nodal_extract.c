/*============================================================================
 * Deal with the exchange of data included in opaque structure fvmc_nodal_t
 *============================================================================*/
...
static inline void
_face_quantities_3d(fvmc_lnum_t          n_vertices,
                    ...
                    double             *surface)
{
  ...
  const fvmc_coord_t *_coords_0, *_coords_1;
  ...
  for (i = 0; i < n_vertices; i++) {
    ...
    fvmc_lnum_t v_id_0 = face_vertex_num[i] - 1;
    fvmc_lnum_t v_id_1 = face_vertex_num[(i+1)%n_vertices] - 1;
...
static inline void
_face_quantities_2d(fvmc_lnum_t          n_vertices,
                    const fvmc_lnum_t    face_vertex_num[],
                    const fvmc_lnum_t   *parent_vertex_num,
                    const fvmc_coord_t   vertex_coords[],
                    double              center[2],
                    double             *surface)
{
  fvmc_lnum_t  i, j, v_id_1, v_id_2;
  double v0[2], v1[2], tc[2], ts;
  const fvmc_coord_t *_coords_0, *_coords_1, *_coords_2;
...
static void
_cell_poly_section_centers(const fvmc_nodal_section_t  *this_section,
                           fvmc_lnum_t                  n_elements,
                           fvmc_lnum_t                 *element_count,
                           const fvmc_lnum_t           *parent_vertex_num,
                           const fvmc_coord_t           vertex_coords[],
                           ...
                           fvmc_coord_t                *cell_centers)
{
  ...
  fvmc_lnum_t _element_count = *element_count;

  assert(this_section != NULL);

  assert(this_section->type == FVMC_CELL_POLY);
  ...
  for (i = 0; i < this_section->n_elements; i++) {
    ...
    for (j = this_section->face_index[i];
      ...
      fvmc_lnum_t face_id = FVMC_ABS(this_section->face_num[j]) - 1;
      fvmc_lnum_t v_index_start = this_section->vertex_index[face_id];
      fvmc_lnum_t v_index_end = this_section->vertex_index[face_id + 1];
      fvmc_lnum_t n_vertices = v_index_end - v_index_start;
...
static void
_cell_strided_section_centers(const fvmc_nodal_section_t  *this_section,
                              fvmc_lnum_t                  n_elements,
                              fvmc_lnum_t                 *element_count,
                              const fvmc_lnum_t           *parent_vertex_num,
                              const fvmc_coord_t           vertex_coords[],
                              ...
                              fvmc_coord_t                *cell_centers)
{
  ...
  fvmc_nodal_cell_face_connect(this_section->type,
                              &n_faces,
                              n_face_vertices,
                              face_vertices);
...
static void
_face_section_centers_3d(const fvmc_nodal_section_t  *this_section,
                         fvmc_lnum_t                  n_elements,
                         fvmc_lnum_t                 *element_count,
                         const fvmc_lnum_t           *parent_vertex_num,
                         const fvmc_coord_t           vertex_coords[],
                         ...
                         fvmc_coord_t                *cell_centers)
{
  fvmc_lnum_t i, v_index_start, v_index_end, n_vertices;
  int k;
...
static void
_face_section_centers_2d(const fvmc_nodal_section_t  *this_section,
                         fvmc_lnum_t                  n_elements,
                         fvmc_lnum_t                 *element_count,
                         const fvmc_lnum_t           *parent_vertex_num,
                         const fvmc_coord_t           vertex_coords[],
                                ...
                                fvmc_gnum_t         *g_vtx_num)
{
  ...
  fvmc_io_num_t *global_io_num = this_nodal->global_vertex_num;
  ...
  if (global_io_num == NULL) {
    ...
    size = sizeof(fvmc_gnum_t) * fvmc_io_num_get_local_count(global_io_num);
    memcpy(g_vtx_num, fvmc_io_num_get_global_num(global_io_num), size);
                                 ...
                                 fvmc_gnum_t         *g_elt_num)
{
  ...
  fvmc_lnum_t n_elements = 0, element_count = 0;
  fvmc_gnum_t n_g_elements = 0, global_count = 0;
  const fvmc_gnum_t *g_num = NULL;
  ...
  for (section_id = 0; section_id < this_nodal->n_sections; section_id++) {
    ...
    if (section->type == element_type) {
      ...
      const fvmc_io_num_t *io_num = section->global_element_num;
      ...
      if (io_num == NULL) {
        ...
        n_elements = fvmc_io_num_get_local_count(io_num);
        n_g_elements = fvmc_io_num_get_global_count(io_num);
        g_num = fvmc_io_num_get_global_num(io_num);

        if (global_count == 0) {
          memcpy((char *)g_elt_num,
                 g_num,
                 sizeof(fvmc_gnum_t) * n_elements);
                            ...
                            fvmc_coord_t        *vertex_coords)
{
  ...
  const fvmc_lnum_t n_vertices = this_nodal->n_vertices;
  const fvmc_coord_t *coords = this_nodal->vertex_coords;
  const fvmc_lnum_t  *parent_num = this_nodal->parent_vertex_num;

  if (this_nodal->parent_vertex_num == NULL) {

    if (interlace == FVMC_INTERLACE)
      memcpy(vertex_coords, coords, sizeof(fvmc_coord_t) * n_vertices * dim);
                              ...
                              fvmc_coord_t        *cell_centers)
{
  ...
  fvmc_lnum_t element_count = 0;

  const int dim = this_nodal->dim;
  const fvmc_coord_t *coords = this_nodal->vertex_coords;
  const fvmc_lnum_t  *parent_num = this_nodal->parent_vertex_num;
  const fvmc_lnum_t n_elements = fvmc_nodal_get_n_entities(this_nodal,
                                                         entity_dim);
                              ...
                              fvmc_lnum_t         *connectivity)
{
  ...
  if (element_type == FVMC_FACE_POLY || element_type == FVMC_CELL_POLY)
    bftc_error(__FILE__, __LINE__, 0,
              _("Elements of type : \"%s\" are not strided elements.\n"
                "Incorrect use with fvmc_nodal_get_strided_connect()\n"
                "Associated nodal mesh : \"%s\"\n"),
              fvmc_elements_type_name[element_type], this_nodal->name);
  ...
  for (section_id = 0; section_id < this_nodal->n_sections; section_id++) {
    ...
    if (section->type == element_type) {
      ...
      const int stride = section->stride;
      const fvmc_lnum_t *num = section->vertex_num;
...
{
  int section_id;
  fvmc_lnum_t i, j, k, l, element_concat_id, vertex_id;

  fvmc_lnum_t *element_count = NULL;
  fvmc_lnum_t *_element_index = NULL;
  fvmc_lnum_t *_element_id = NULL;
  ...
  *element_id = NULL;
  ...
  BFTC_MALLOC(element_count, n_vertices, fvmc_lnum_t);
  for (i = 0; i < n_vertices; i++)
    element_count[i] = 0;
  ...
  for (section_id = 0; section_id < this_nodal->n_sections; section_id++) {
    ...
    if (section->type != FVMC_FACE_POLY && section->type != FVMC_CELL_POLY) {
      ...
      for (i = 0; i < section->n_elements; i++) {
        fvmc_lnum_t start_id = section->vertex_index[i];
        fvmc_lnum_t end_id = section->vertex_index[i+1];
      ...
      for (i = 0; i < section->n_elements; i++) {
        fvmc_lnum_t start_face_id = section->face_index[i];
        fvmc_lnum_t end_face_id = section->face_index[i+1];
        for (j = start_face_id; j < end_face_id; j++) {
          fvmc_lnum_t face_id = FVMC_ABS(section->face_num[j]) - 1;
          fvmc_lnum_t start_id = section->vertex_index[face_id];
          fvmc_lnum_t end_id = section->vertex_index[face_id+1];
  ...
  BFTC_MALLOC(_element_index, n_vertices + 1, fvmc_lnum_t);
  _element_index[0] = 0;
  ...
  BFTC_MALLOC(_element_id, _element_index[n_vertices], fvmc_lnum_t);
  ...
  assert(element_concat_id == fvmc_nodal_get_n_entities(this_nodal, entity_dim));
