...
/*----------------------------------------------------------------------------
 * Descend binary tree for the ordering of a fvmc_gnum (integer) array.
 *
 * parameters:
...
inline static void
_order_gnum_descend_tree(const fvmc_gnum_t  number[],
                         size_t            level,
                         const size_t      nb_ent,
                         fvmc_lnum_t        order[])
{
...
static void
_order_local(const fvmc_gnum_t  number[],
             fvmc_lnum_t        order[],
             const size_t      nb_ent)
{
  size_t i;
...
inline static void
_order_gnum_descend_tree_s(const fvmc_gnum_t  number[],
                           size_t            stride,
                           size_t            level,
                           const size_t      nb_ent,
                           fvmc_lnum_t        order[])
...
static void
_order_local_s(const fvmc_gnum_t  number[],
               size_t            stride,
               fvmc_lnum_t        order[],
               const size_t      nb_ent)
{
...
inline static void
_order_gnum_descend_tree_i(const fvmc_gnum_t  number[],
                           const fvmc_lnum_t  index[],
                           size_t            level,
                           const size_t      nb_ent,
                           fvmc_lnum_t        order[])
...
static void
_order_local_i(const fvmc_gnum_t  number[],
               const fvmc_lnum_t  index[],
               fvmc_lnum_t        order[],
               const size_t      nb_ent)
{
...
int
fvmc_order_local_test(const fvmc_lnum_t  list[],
                     const fvmc_gnum_t  number[],
                     const size_t      nb_ent)
{
  size_t i = 0;
...
int
fvmc_order_local_test_s(const fvmc_lnum_t  list[],
                       const fvmc_gnum_t  number[],
                       size_t            stride,
                       size_t            nb_ent)
{
...
fvmc_lnum_t *
fvmc_order_local(const fvmc_lnum_t  list[],
                const fvmc_gnum_t  number[],
                size_t            nb_ent)
{
  fvmc_lnum_t *order;

  BFTC_MALLOC(order, nb_ent, fvmc_lnum_t);

  fvmc_order_local_allocated(list,
                            number,
                            order,
                            nb_ent);
...
fvmc_lnum_t *
fvmc_order_local_s(const fvmc_lnum_t  list[],
                  const fvmc_gnum_t  number[],
                  size_t            stride,
                  size_t            nb_ent)
{
  ...
  fvmc_order_local_allocated_s(list,
                              number,
                              stride,
                              order,
                              nb_ent);
...
fvmc_lnum_t *
fvmc_order_local_i(const fvmc_lnum_t  list[],
                  const fvmc_gnum_t  number[],
                  const fvmc_lnum_t  index[],
                  size_t            nb_ent)
{
  ...
  fvmc_order_local_allocated_i(list, number, index, order, nb_ent);
...
void
fvmc_order_local_allocated(const fvmc_lnum_t  list[],
                          const fvmc_gnum_t  number[],
                          fvmc_lnum_t        order[],
                          const size_t      nb_ent)
{
  size_t i;
  fvmc_gnum_t *number_list;
  ...
  if (number != NULL) {
    ...
    if (list != NULL) {
      BFTC_MALLOC(number_list, nb_ent, fvmc_gnum_t);
      for (i = 0 ; i < nb_ent ; i++)
        number_list[i] = number[list[i] - 1];
      _order_local(number_list,
                   order,
                   nb_ent);
      BFTC_FREE(number_list);
    }
    else
      _order_local(number,
                   order,
  ...
  /* Implicit numbering */
    ...
    if (list != NULL) {
      for (i = 0 ; i < nb_ent ; i++)
        number_list[i] = (fvmc_gnum_t)(list[i]);
    }
    else {
      for (i = 0 ; i < nb_ent ; i++)
        number_list[i] = (fvmc_gnum_t)(i + 1);
    }
    _order_local(number_list,
                 order,
                 nb_ent);
...
void
fvmc_order_local_allocated_s(const fvmc_lnum_t  list[],
                            const fvmc_gnum_t  number[],
                            size_t            stride,
                            fvmc_lnum_t        order[],
                            const size_t      nb_ent)
{
  ...
  if (number != NULL) {
    ...
    if (list != NULL) {
      BFTC_MALLOC(number_list, nb_ent*stride, fvmc_gnum_t);
      for (i = 0 ; i < nb_ent ; i++) {
        for (j = 0; j < stride; j++)
...
void
fvmc_order_local_allocated_i(const fvmc_lnum_t  list[],
                            const fvmc_gnum_t  number[],
                            const fvmc_lnum_t  index[],
                            fvmc_lnum_t        order[],
                            const size_t      nb_ent)
{
  ...
  if (number != NULL) {
    ...
    if (list != NULL) {
      ...
      fvmc_gnum_t  *number_list = NULL;

      BFTC_MALLOC(_index, nb_ent + 1, fvmc_lnum_t);
      ...
      BFTC_MALLOC(number_list, _index[nb_ent], fvmc_gnum_t);
      ...
      for (i = 0; i < nb_ent; i++) {
        ...
        for (j = index[ent_id], k = 0;
             (fvmc_lnum_t)j < index[ent_id+1]; j++, k++)
  ...
  /* Implicit numbering */
    ...
    fvmc_order_local_allocated(list, NULL, order, nb_ent);
...
fvmc_lnum_t *
fvmc_order_local_renumbering(const fvmc_lnum_t  order[],
                            const size_t      nb_ent)
{
  size_t i;
  fvmc_lnum_t *number;
  ...
  BFTC_MALLOC(number, nb_ent, fvmc_lnum_t);
