...
struct _fvmc_interface_t {

  int          rank;           /* Associated rank */

  fvmc_lnum_t   size;           /* Number of equivalent elements */

  int          tr_index_size;  /* Size of perio_index */
  fvmc_lnum_t  *tr_index;       /* Index of sub-sections in local_num and
  ...
  fvmc_lnum_t  *local_num;     /* Local element numbers */
  fvmc_lnum_t  *distant_num;   /* Distant element numbers */
...
struct _fvmc_interface_set_t {

  int                        size;         /* Number of interfaces */

  fvmc_interface_t          **interfaces;   /* Interface structures array */

  const fvmc_periodicity_t   *periodicity;  /* Optional periodicity structure */
...
typedef struct {
  ...
  fvmc_lnum_t  *num;      /* Local number associated with each element */
...
typedef struct {
  ...
  fvmc_lnum_t  *slice_id;  /* local id in slice */
  ...
  fvmc_lnum_t  *num;       /* Local numbers associated with periodic elements */
...
static fvmc_interface_t *
_fvmc_interface_create(void)
{
  fvmc_interface_t  *_interface;

  BFTC_MALLOC(_interface, 1, fvmc_interface_t);
...
static fvmc_interface_t *
_fvmc_interface_destroy(fvmc_interface_t  *this_interface)
{
  if (this_interface != NULL) {
    BFTC_FREE(this_interface->tr_index);
    BFTC_FREE(this_interface->local_num);
    BFTC_FREE(this_interface->distant_num);
    BFTC_FREE(this_interface);
  }
...
static void
_fvmc_interface_dump(const fvmc_interface_t  *this_interface)
{
  int i, section_id;
  fvmc_lnum_t start_id, end_id;

  int  tr_index_size = 2;
  fvmc_lnum_t  _tr_index[2] = {0, 0};
  const fvmc_lnum_t  *tr_index = _tr_index;

  if (this_interface == NULL) {
    bftc_printf("  interface: nil\n");
    return;
  }

  bftc_printf("  interface:             %p\n"
             "  associated rank:       %d\n"
             "  size:                  %lu\n"
...
static void
_sort_periodic_couples(fvmc_lnum_t   *n_slice_couples,
                       fvmc_gnum_t  **slice_couples)
{
  fvmc_lnum_t  i, j, k;

  fvmc_lnum_t   n_couples = *n_slice_couples;
  fvmc_lnum_t  *order = NULL;
  fvmc_gnum_t  *couples = *slice_couples;
  fvmc_gnum_t  *couples_tmp = NULL;
  ...
  BFTC_MALLOC(order, n_couples, fvmc_lnum_t);
  BFTC_MALLOC(couples_tmp, n_couples*3, fvmc_gnum_t);
  ...
  if (n_couples <= *n_slice_couples) {
    BFTC_REALLOC(couples, n_couples*3, fvmc_gnum_t);
  ...
  memcpy(couples, couples_tmp, sizeof(fvmc_gnum_t)*n_couples*3);

  BFTC_FREE(couples_tmp);
}
...
static void
_transform_combine_info(const fvmc_periodicity_t   *periodicity,
                        int                        level,
                        int                       *n_rows,
                        int                      **tr_combine)
{
  ...
  fvmc_periodicity_get_tr_level_idx(periodicity, tr_level_idx);
  ...
  BFTC_MALLOC(tr_combine_1, n_vals_1, int);
  ...
  for (i = tr_level_idx[1]; i < tr_level_idx[2]; i++) {
    ...
    fvmc_periodicity_get_parent_ids(periodicity, i, parent_id);
  ...
  /* Handle level 2 transforms */
    ...
    BFTC_MALLOC(tr_combine_2, n_vals_2, int);
    ...
    for (i = tr_level_idx[2]; i < tr_level_idx[3]; i++) {
      ...
      fvmc_periodicity_get_components(periodicity, i, comp_id);
...
static fvmc_gnum_t
_global_num_max(fvmc_lnum_t        n_ent,
...
_slice_global_num_to_equiv(int                n_ranks,
                           fvmc_lnum_t         n_ent_recv,
                           const fvmc_lnum_t   recv_shift[],
                           const fvmc_gnum_t   recv_global_num[],
                           const fvmc_lnum_t   recv_num[],
                           fvmc_lnum_t         equiv_id[])
{
  fvmc_lnum_t  i, j;
  int         rank;
  fvmc_gnum_t  cur_num, prev_num;
  ...
  BFTC_MALLOC(multiple, e.count, int);

  BFTC_MALLOC(e.shift, e.count+1, fvmc_lnum_t);
  ...
  BFTC_MALLOC(e.rank, e.shift[e.count], fvmc_lnum_t);
  BFTC_MALLOC(e.num, e.shift[e.count], fvmc_lnum_t);
...
static void
_interfaces_from_flat_equiv(fvmc_interface_set_t  *this_interface_set,
                            int                   tr_index_size,
                            fvmc_lnum_t            n_ent_recv,
                            const fvmc_lnum_t      equiv_recv[])
{
  fvmc_lnum_t i, j, k, l;
  fvmc_lnum_t local_num, distant_num, n_sub, n_ent_rank_tr_size;
  int rank, tr_id;
  ...
  fvmc_lnum_t  *n_ent_rank = NULL;
  fvmc_lnum_t  *n_ent_rank_tr = NULL;
  int *interface_id = NULL;

  fvmc_interface_t *_interface = NULL;
  ...
  BFTC_MALLOC(n_ent_rank, max_rank + 1, fvmc_lnum_t);
  ...
  BFTC_REALLOC(this_interface_set->interfaces,
              this_interface_set->size,
              fvmc_interface_t *);

  for (i = start_id; i < this_interface_set->size; i++)
    this_interface_set->interfaces[i] = _fvmc_interface_create();

  /* Initialize rank info and interface id */

  n_ranks = 0;
  BFTC_MALLOC(interface_id, max_rank + 1, int);
  for (i = 0; i < max_rank + 1; i++) {
    if (n_ent_rank[i] > 0) {
  ...
  BFTC_MALLOC(n_ent_rank_tr, n_ent_rank_tr_size, fvmc_lnum_t);
  ...
  for (i = start_id; i < this_interface_set->size; i++) {
    ...
    BFTC_MALLOC(_interface->local_num, _interface->size, fvmc_lnum_t);
    BFTC_MALLOC(_interface->distant_num, _interface->size, fvmc_lnum_t);

    if (_tr_index_size > 1) {
      _interface->tr_index_size = _tr_index_size;
      BFTC_MALLOC(_interface->tr_index, _interface->tr_index_size, fvmc_lnum_t);
  ...
  BFTC_FREE(n_ent_rank_tr);
  BFTC_FREE(interface_id);
}
...
static void
_fvmc_interface_add_global_equiv(fvmc_interface_set_t  *this_interface_set,
                                fvmc_lnum_t            n_ent,
                                fvmc_gnum_t            global_num[],
                                MPI_Comm              comm)
{
  fvmc_gnum_t  global_max;
  fvmc_lnum_t  i, j, n_ent_recv, n_ent_send;
  size_t      slice_size;
  int         size, rank;
  ...
  fvmc_lnum_t  *equiv_id = NULL;
  fvmc_lnum_t  *equiv_send, *equiv_recv = NULL;
  fvmc_lnum_t  *recv_num = NULL, *send_num = NULL;
  ...
  BFTC_MALLOC(recv_num, n_ent_recv, fvmc_lnum_t);
  ...
  BFTC_MALLOC(send_num, n_ent_send, fvmc_lnum_t);

  for (i = 0; i < n_ent_send; i++)
    send_num[i] = i+1;

  MPI_Alltoallv(send_num, send_count, send_shift, FVMC_MPI_LNUM,
                recv_num, recv_count, recv_shift, FVMC_MPI_LNUM, comm);
  ...
  if (n_ent_recv > 0)
    BFTC_MALLOC(equiv_id, n_ent_recv, fvmc_lnum_t);
  ...
  n_ent_send = send_shift[size];
  BFTC_MALLOC(equiv_send, n_ent_send, fvmc_lnum_t);
  ...
  BFTC_MALLOC(equiv_recv, n_ent_recv, fvmc_lnum_t);

  MPI_Alltoallv(equiv_send, send_count, send_shift, FVMC_MPI_LNUM,
                equiv_recv, recv_count, recv_shift, FVMC_MPI_LNUM, comm);

  /* At this stage, MPI operations are finished; we may release
     the corresponding counts and indexes */

  BFTC_FREE(equiv_send);
                 ...
                 MPI_Comm  comm)
{
  ...
  if (n_counts == 2) {
    ...
    BFTC_MALLOC(count_tmp, size, int);
                                ...
                                MPI_Comm                  comm)
{
  ...
  for (list_id = 0; list_id < n_periodic_lists; list_id++) {
    ...
    fvmc_lnum_t couple_id;
    fvmc_gnum_t num_1, num_2;
    int rank_1, rank_2;

    const fvmc_lnum_t  _n_periodic_couples = n_periodic_couples[list_id];
    const fvmc_gnum_t  *_periodic_couples = periodic_couples[list_id];
                          ...
                          MPI_Comm                   comm)
{
  ...
  fvmc_lnum_t  j, k, start_id, end_id;

  int  *send_count = NULL, *recv_count = NULL;
  int  *send_shift = NULL, *recv_shift = NULL;
  int  *tr_reverse_id = NULL;

  fvmc_gnum_t  *send_couples = NULL;

  fvmc_lnum_t  _n_slice_couples = *n_slice_couples;
  fvmc_gnum_t  *_slice_couples = *slice_couples;
  ...
  n_tr = fvmc_periodicity_get_n_transforms(periodicity);

  BFTC_MALLOC(tr_reverse_id, n_tr, int);

  for (i = 0; i < n_tr; i++)
    tr_reverse_id[i] = fvmc_periodicity_get_reverse_id(periodicity, i);
  ...
  for (level = 1;
       level < fvmc_periodicity_get_n_levels(periodicity);
       level++) {
      ...
      if (_slice_couples[start_id*3] == _slice_couples[end_id*3]) {
        ...
        for (j = start_id; j < end_id; j++) {
          for (k = j+1; k < end_id; k++) {
            ...
            fvmc_gnum_t num_1 = _slice_couples[j*3 + 1];
            fvmc_gnum_t num_2 = _slice_couples[k*3 + 1];
    ...
    BFTC_MALLOC(send_couples, send_shift[size], fvmc_gnum_t);
    ...
    BFTC_FREE(tr_combine);

    n_recv = recv_shift[size] - (_n_slice_couples*3);

    if (n_recv > 0)
      BFTC_REALLOC(_slice_couples, recv_shift[size], fvmc_gnum_t);

    MPI_Alltoallv(send_couples, send_count, send_shift, FVMC_MPI_GNUM,
                  _slice_couples, recv_count, recv_shift, FVMC_MPI_GNUM, comm);

    BFTC_FREE(send_couples);
                           ...
                           MPI_Comm                  comm)
{
  ...
  fvmc_lnum_t   _n_slice_couples = 0;
  int         *send_shift = NULL, *recv_shift = NULL;
  fvmc_gnum_t  *send_couples = NULL, *recv_couples = NULL;
  ...
  *slice_couples = NULL;
  ...
  BFTC_MALLOC(send_couples, send_shift[size], fvmc_gnum_t);
  BFTC_MALLOC(recv_couples, recv_shift[size], fvmc_gnum_t);
  ...
  for (list_id = 0; list_id < n_periodic_lists; list_id++) {
    ...
    const int direct_id = fvmc_periodicity_get_transform_id(periodicity,
                                                           external_num,
                                                           1);
    const int reverse_id = fvmc_periodicity_get_transform_id(periodicity,
                                                            external_num,
                                                            -1);
  ...
  MPI_Alltoallv(send_couples, send_count, send_shift, FVMC_MPI_GNUM,
                recv_couples, recv_count, recv_shift, FVMC_MPI_GNUM, comm);
...
static void
_periodic_couples_slice_id(fvmc_lnum_t         n_slice_elements,
                           const fvmc_lnum_t   order[],
                           const fvmc_gnum_t   slice_global_num[],
                           fvmc_lnum_t         n_slice_couples,
                           int                stride,
                           const fvmc_gnum_t   slice_couples[],
                           fvmc_lnum_t         couple_slice_id[])
{
  fvmc_lnum_t   couple_id;
  ...
  for (couple_id = 0; couple_id < n_slice_couples; couple_id++) {
    ...
    fvmc_lnum_t end_id = n_slice_elements - 1;
    fvmc_lnum_t mid_id = (end_id -start_id) / 2;

    const fvmc_gnum_t num_1 = slice_couples[couple_id*stride];
...
_rank_by_slice_id(int                   n_slices,
                  const fvmc_lnum_t      slice_shift[],
                  fvmc_lnum_t            slice_id)
{
  fvmc_lnum_t start_id = 0;
  fvmc_lnum_t end_id = n_slices - 1;
  fvmc_lnum_t mid_id = (end_id -start_id) / 2;
                               ...
                               MPI_Comm                   comm)
{
  ...
  if (equiv != NULL && equiv_id != NULL) {
    ...
    for (couple_id = 0; couple_id < n_slice_couples; couple_id++) {
      ...
      int e_mult;
      fvmc_gnum_t num_2 = slice_couples[couple_id*3 + 1];
      fvmc_lnum_t e_id = equiv_id[couple_slice_id[couple_id]];
      int rank_2 = (num_2 - 1) / slice_size;
                         ...
                         MPI_Comm                   comm)
{
  ...
  int          size;
  int          rank;
  fvmc_lnum_t   n_slice_elements, couple_id;
  ...
  int *recv_shift = NULL;
  fvmc_lnum_t *order = NULL;
  fvmc_gnum_t *equiv_send = NULL, *equiv_recv = NULL;
  fvmc_gnum_t *slice_recv_num = NULL;
  ...
  order = fvmc_order_local(NULL, slice_global_num, n_slice_elements);

  /* Associate id in slice for periodic couples prior to sending */

  BFTC_MALLOC(couple_slice_id, n_slice_couples, fvmc_lnum_t);
  ...
  /* arrays to exchange; most of the exchanged data is of type int or
     fvmc_lnum_t, using only positive values. For each periodic couple,
     one value of type fvmc_gnum_t is exchanged, so to group all communication
     in one MPI call, all data is exchanged as fvmc_gnum_t, even if this
     means larger messages if fvmc_gnum_t is larger than fvmc_lnum_t.
     As the number of elements per couple is variable (depending on prior
     equivalence info), using an MPI datatype to mix int and fvmc_gnum
     types rather than casting all to fvmc_gnum_t is not feasible */

  BFTC_MALLOC(equiv_send, send_shift[size], fvmc_gnum_t);
  BFTC_MALLOC(equiv_recv, recv_shift[size], fvmc_gnum_t);

  /* temporary array to find reverse transforms */

  n_tr = fvmc_periodicity_get_n_transforms(periodicity);

  BFTC_MALLOC(reverse_tr_id, n_tr, int);

  for (tr_id = 0; tr_id < n_tr; tr_id++)
    reverse_tr_id[tr_id] = fvmc_periodicity_get_reverse_id(periodicity, tr_id);
  ...
  if (equiv != NULL && equiv_id != NULL) {
    ...
    for (couple_id = 0; couple_id < n_slice_couples; couple_id++) {
      ...
      const fvmc_gnum_t num_2 = slice_couples[couple_id*3 + 1];
      const fvmc_lnum_t local_id = couple_slice_id[couple_id];
      const fvmc_lnum_t e_id = equiv_id[local_id];
  ...
  BFTC_FREE(couple_slice_id);
  BFTC_FREE(reverse_tr_id);

  /* Parallel exchange */

  MPI_Alltoallv(equiv_send, send_count, send_shift, FVMC_MPI_GNUM,
                equiv_recv, recv_count, recv_shift, FVMC_MPI_GNUM, comm);
...

  {
    ...
    BFTC_MALLOC(slice_recv_num, pe.count, fvmc_gnum_t);

    BFTC_MALLOC(pe.tr_id, pe.count, int);

    BFTC_MALLOC(pe.shift, pe.count + 1, int);

    BFTC_MALLOC(pe.rank, j, int);
    BFTC_MALLOC(pe.num, j, int);
  ...
  BFTC_MALLOC(pe.slice_id, pe.count, fvmc_lnum_t);
                      ...
                      _per_slice_period_t  *perio_equiv)
{
  ...
  BFTC_MALLOC(eq_mult, new_count, int);
  ...
  BFTC_MALLOC(new_shift, new_count+1, int);
  ...
  if (old_count > 0) {
    ...
    BFTC_MALLOC(new_rank, new_size, int);
    ...
    BFTC_FREE(equiv->rank);
    equiv->rank = new_rank;

    BFTC_MALLOC(new_num, new_size, int);
    ...
    if (equiv->tr_id != NULL) {
      ...
      int *new_tr_id = NULL;
      BFTC_MALLOC(new_tr_id, new_size, int);
      for (i = 0; i < old_count; i++) {
        for (k = 0; k < eq_mult[i]; k++)
          new_tr_id[new_shift[i] + k] = equiv->tr_id[equiv->shift[i] + k];
      }
      BFTC_FREE(equiv->tr_id);
    ...
    BFTC_FREE(equiv->shift);
    equiv->shift = new_shift;
    BFTC_MALLOC(equiv->rank, new_size, int);
    BFTC_MALLOC(equiv->num, new_size, int);

  }

  if (equiv->tr_id == NULL) {
    BFTC_MALLOC(equiv->tr_id, new_size, int);
  ...
  for (i = 0; i < pe->count; i++) {
    ...
    int k, l;
    const fvmc_lnum_t _slice_id = pe->slice_id[i];
    const int eq_id = equiv_id[_slice_id];
...
static void
_add_global_equiv_periodic(fvmc_interface_set_t      *this_interface_set,
                           fvmc_lnum_t                n_ent,
                           fvmc_gnum_t                global_num[],
                           const fvmc_periodicity_t  *periodicity,
                           int                       n_periodic_lists,
                           ...
                           MPI_Comm                  comm)
{
  ...
  int         n_counts = 2;
  fvmc_lnum_t  n_slice_couples = 0;
  int         *send_count = NULL, *recv_count = NULL;
  int         *send_shift = NULL, *recv_shift = NULL;
  fvmc_lnum_t  *equiv_id = NULL, *couple_equiv_id = NULL;
  fvmc_lnum_t  *equiv_send = NULL, *equiv_recv = NULL;
  fvmc_lnum_t  *recv_num = NULL, *send_num = NULL;
  fvmc_gnum_t  *recv_global_num = NULL;
  fvmc_gnum_t  *slice_couples = NULL;

  /* Initialization */

  MPI_Comm_size(comm, &size);

  tr_index_size = fvmc_periodicity_get_n_transforms(periodicity) + 2;
  ...
  BFTC_MALLOC(send_count, size*n_counts, int);
  BFTC_MALLOC(recv_count, size*n_counts, int);
  ...
  BFTC_REALLOC(send_count, size, int);
  BFTC_REALLOC(recv_count, size, int);

  /* Combine periodic couples if necessary */

  if (fvmc_periodicity_get_n_levels(periodicity) > 1)
  ...
  BFTC_FREE(couple_equiv_id);
  BFTC_FREE(slice_couples);
  n_slice_couples = 0;
...
static void
_define_periodic_couples_sp(const fvmc_periodicity_t  *periodicity,
                            int                       n_periodic_lists,
                            const int                 periodicity_num[],
                            const fvmc_lnum_t          n_periodic_couples[],
                            const fvmc_gnum_t   *const periodic_couples[],
                            fvmc_lnum_t               *n_couples,
                            fvmc_gnum_t              **couples)
{
  int         list_id;

  fvmc_lnum_t  count = 0;
  fvmc_lnum_t   _n_couples = 0;
  fvmc_gnum_t  *_couples = NULL;
  ...
  *couples = NULL;
  ...
  BFTC_MALLOC(_couples, _n_couples*3, fvmc_gnum_t);
...
static void
_combine_periodic_couples_sp(const fvmc_periodicity_t   *periodicity,
                             fvmc_lnum_t                *n_couples,
                             fvmc_gnum_t               **couples)
{
  int  i, n_tr, level;

  fvmc_lnum_t  start_id, end_id, j, k;

  int  add_count = 0;
  int  *tr_reverse_id = NULL;
  fvmc_gnum_t *_add_couples = NULL;

  fvmc_lnum_t  _n_couples = *n_couples;
  fvmc_gnum_t  *_couples = *couples;
  ...
  for (level = 1;
    ...
    BFTC_REALLOC(_couples, _n_couples*3 + add_count, fvmc_gnum_t);
      ...
      if (_couples[start_id*3] == _couples[end_id*3]) {
        ...
        for (j = start_id; j < end_id; j++) {
          for (k = j+1; k < end_id; k++) {
            ...
            fvmc_gnum_t num_1 = _couples[j*3 + 1];
            fvmc_gnum_t num_2 = _couples[k*3 + 1];
            int tr_1 = tr_reverse_id[_couples[j*3 + 2]];
            int tr_2 = _couples[k*3 + 2];
            int tr_c = tr_combine[tr_1 *n_rows + tr_2];
...
static void
_add_global_equiv_periodic_sp(fvmc_interface_set_t      *this_interface_set,
                              const fvmc_periodicity_t  *periodicity,
                              int                       n_periodic_lists,
                              const int                 periodicity_num[],
                              const fvmc_lnum_t          n_periodic_couples[],
                              const fvmc_gnum_t   *const periodic_couples[])
{
  fvmc_lnum_t  i, couple_id;
  fvmc_lnum_t  n_couples = 0;
  fvmc_lnum_t  *n_ent_tr = NULL;
  fvmc_gnum_t  *couples = NULL;
  ...
  _interface = _fvmc_interface_create();
  ...
  _interface->tr_index_size
    = fvmc_periodicity_get_n_transforms(periodicity) + 2;
  ...
  BFTC_MALLOC(n_ent_tr, _interface->tr_index_size - 2, fvmc_lnum_t);
  ...
  for (couple_id = 0; couple_id < n_couples; couple_id++) {
    ...
    const fvmc_lnum_t j = _interface->tr_index[tr_id + 1] + n_ent_tr[tr_id];
...
fvmc_interface_set_create(fvmc_lnum_t                n_ent,
                         ...
                         const fvmc_gnum_t   *const periodic_couples[])
{
  fvmc_interface_set_t  *this_interface_set;
  ...
  BFTC_MALLOC(this_interface_set, 1, fvmc_interface_set_t);
  ...
  if (fvmc_parall_get_size() > 1) {

    size_t  i;
    fvmc_gnum_t  *global_num = NULL;

    if (n_ent > 0) {

      BFTC_MALLOC(global_num, n_ent, fvmc_gnum_t);
    ...
    if (fvmc_parall_get_size() > 1 && periodicity == NULL)

      _fvmc_interface_add_global_equiv(this_interface_set,
                                      n_ent,
                                      global_num,
                                      fvmc_parall_get_mpi_comm());

    else if (fvmc_parall_get_size() > 1)
...
fvmc_interface_set_destroy(fvmc_interface_set_t  *this_interface_set)
{
  ...
  if (this_interface_set != NULL) {
    for (i = 0; i < this_interface_set->size; i++) {
      _fvmc_interface_destroy(this_interface_set->interfaces[i]);
    }
    BFTC_FREE(this_interface_set->interfaces);
    BFTC_FREE(this_interface_set);
  }
...
fvmc_interface_set_get(const fvmc_interface_set_t  *this_interface_set,
                      int                         interface_id)
{
  const fvmc_interface_t  *retval = NULL;
...
{
  const fvmc_periodicity_t  *retval = NULL;
...
fvmc_interface_set_dump(const fvmc_interface_set_t  *this_interface_set)
{
  ...
  if (this_interface_set == NULL) {
    bftc_printf("  interface list: nil\n");
    return;
  }

  bftc_printf("  interface list: %p\n"
             "  n interfaces:   %d\n",
             this_interface_set, this_interface_set->size);

  for (i = 0; i < this_interface_set->size; i++) {
    bftc_printf("\n  interface %d:\n", i);
    _fvmc_interface_dump(this_interface_set->interfaces[i]);
  }
