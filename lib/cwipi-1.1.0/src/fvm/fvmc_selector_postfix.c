...
struct _fvmc_selector_postfix_t {
              ...
              const char        **keywords)
{
  ...
  BFTC_REALLOC(this_parser->operators,
  ...
  if (n_keywords > 0) {
    ...
    BFTC_REALLOC(this_parser->keyword_op_id,
                this_parser->n_keywords + n_keywords,
                int);

    BFTC_REALLOC(this_parser->keyword,
                this_parser->n_keywords + n_keywords,
                char *);

    BFTC_REALLOC(this_parser->keywords,
...
_parser_destroy(_parser_t  **this_parser)
{
  if (*this_parser != NULL) {
    ...
    BFTC_FREE((*this_parser)->operators);
    BFTC_FREE((*this_parser)->keyword_op_id);
    BFTC_FREE((*this_parser)->keyword);
    BFTC_FREE((*this_parser)->keywords);

    BFTC_FREE(*this_parser);
...
_tokenize(const char  *infix)
{
  ...
  BFTC_MALLOC(te.infix_id, l, int);
  BFTC_MALLOC(te.token_id, l, int);
  BFTC_MALLOC(te._protected, l, _Bool);
  BFTC_MALLOC(te.tokens, te.max_size, char);
  ...
  BFTC_REALLOC(te.infix_id, te.n_tokens, int);
  BFTC_REALLOC(te.token_id, te.n_tokens, int);
  BFTC_REALLOC(te._protected, te.n_tokens, _Bool);
  BFTC_REALLOC(te.tokens, te.size, char);
...
_empty_tokenized(_tokenized_t  *te)
{
  ...
  te->size = 0;
  te->max_size = 0;
  BFTC_FREE(te->infix_id);
  BFTC_FREE(te->token_id);
  BFTC_FREE(te->_protected);
  BFTC_FREE(te->tokens);
}
                ...
                const _tokenized_t   te)
{
  ...
  for (i = 0; i < te.n_tokens; i++) {
    bftc_printf("  %3d: %-20s", i, te.tokens + te.token_id[i]);
    bftc_printf(" (%d bytes from infix start", te.infix_id[i]);
    if (te._protected[i] != 0)
      bftc_printf(", protected)\n");
    else
      bftc_printf(")\n");
  }
}
            ...
            int                 token_id)
{
  ...
  if (stack->size >=  stack->max_size) {
    ...
    if (stack->max_size > BASE_STACK_SIZE) {
      if (stack->elements == stack->_elements) {
        BFTC_MALLOC(stack->elements, stack->max_size, _stack_entry_t);
        memcpy(stack->elements, stack->_elements, stack->size*sizeof(_stack_entry_t));
      }
      else
        BFTC_REALLOC(stack->elements, stack->max_size, _stack_entry_t);
    }
  }
...
static fvmc_selector_postfix_t *
_postfix_create(const char *infix)
{
  ...
  BFTC_MALLOC(pf, 1, fvmc_selector_postfix_t);
  ...
  BFTC_MALLOC(pf->infix, size + 1, char);
  strcpy(pf->infix, infix);

  BFTC_MALLOC(pf->elements, pf->max_size, unsigned char);
...
static void
_postfix_destroy(fvmc_selector_postfix_t **pf)
{
  fvmc_selector_postfix_t *_pf = *pf;

  if (*pf != NULL) {

    BFTC_FREE(_pf->infix);
    BFTC_FREE(_pf->elements);

    if (_pf->n_missing_operands > 0) {
      int i;
      for (i = 0; i < _pf->n_missing_operands; i++)
        BFTC_FREE(_pf->missing_operand[i]);
      BFTC_FREE(_pf->missing_operand);
    }
...
static void
_postfix_grow(fvmc_selector_postfix_t  *pf,
              size_t                   new_size)
{
  size_t i;
  ...
  BFTC_REALLOC(pf->elements, pf->max_size, unsigned char);
...
static void
_postfix_adjust(fvmc_selector_postfix_t  *pf)
{
...
static void
_postfix_add_opcode(fvmc_selector_postfix_t  *pf,
...
static void
_postfix_add_int(fvmc_selector_postfix_t  *pf,
                 int                      val,
...
static void
_postfix_add_float(fvmc_selector_postfix_t  *pf,
                   double                   val)
{
...
static void
_postfix_add_missing(fvmc_selector_postfix_t  *pf,
                     const char              *missing)
{
  int n = pf->n_missing_operands;

  BFTC_REALLOC(pf->missing_operand, n + 1, char *);
  BFTC_MALLOC(pf->missing_operand[n], strlen(missing) + 1, char);
...
_parse_error(const char               *err_str,
             ...
             int                       token_id,
             _stack_t                 *stack,
             fvmc_selector_postfix_t  **postfix)
{
  int infix_pos = -1;
  ...
  if (infix_pos > -1) {
    ...
    BFTC_MALLOC(infix_string_marker, infix_pos + 2, char);
...
static inline _Bool
_eval_normal(const fvmc_selector_postfix_t  *pf,
             const double                   normal[],
             size_t                        *i)
{
  double dotp;
...
static inline _Bool
_eval_plane(const fvmc_selector_postfix_t  *pf,
            const double                   coords[],
            size_t                        *i)
{
  double pfunc;
...
static inline _Bool
_eval_box(const fvmc_selector_postfix_t  *pf,
          const double                   coords[],
          size_t                        *i)
{
  double val[12];
...
static inline _Bool
_eval_cylinder(const fvmc_selector_postfix_t  *pf,
               const double                   coords[],
               size_t                        *i)
{
  double val[7];
...
static inline _Bool
_eval_sphere(const fvmc_selector_postfix_t  *pf,
             const double                   coords[],
             size_t                        *i)
{
  double val[4];
...
static inline _Bool
_eval_coord_gt(const fvmc_selector_postfix_t  *pf,
               const double                   coords[],
               size_t                        *i)
{
  double cmp_val;
...
static inline _Bool
_eval_coord_lt(const fvmc_selector_postfix_t  *pf,
               const double                   coords[],
               size_t                        *i)
{
  double cmp_val;
...
static inline _Bool
_eval_coord_ge(const fvmc_selector_postfix_t  *pf,
               const double                   coords[],
               size_t                        *i)
{
  double cmp_val;
...
static inline _Bool
_eval_coord_le(const fvmc_selector_postfix_t  *pf,
               const double                   coords[],
               size_t                        *i)
{
  double cmp_val;
...
fvmc_selector_postfix_create(const char  *infix,
                            int          n_groups,
                            int          n_attributes,
                            const char  *group_name[],
                            const int    attribute[])
...
void
fvmc_selector_postfix_destroy(fvmc_selector_postfix_t  **postfix)
{
  assert(postfix != NULL);
  assert(*postfix != NULL);
...
const char *
fvmc_selector_postfix_get_infix(const fvmc_selector_postfix_t  *pf)
{
  assert(pf != NULL);
...
_Bool
fvmc_selector_postfix_coords_dep(const fvmc_selector_postfix_t  *pf)
{
  assert(pf != NULL);
...
_Bool
fvmc_selector_postfix_normals_dep(const fvmc_selector_postfix_t  *pf)
{
  assert(pf != NULL);
...
int
fvmc_selector_postfix_n_missing(const fvmc_selector_postfix_t  *pf)
{
  assert(pf != NULL);
 ...
 * parameters:
 *   pf <-- pointer to postfix structure
 *   id <-- id of missing operand (0 to fvmc_selector_postfix_n_missing())
 *
 * returns:
...
const char *
fvmc_selector_postfix_get_missing(const fvmc_selector_postfix_t  *pf,
                                 int                            id)
{
...
_Bool
fvmc_selector_postfix_eval(const fvmc_selector_postfix_t  *pf,
                          int                            n_groups,
                          int                            n_attributes,
                          const int                      group_id[],
  ...
  while (i < pf->size) {
    ...
    case PF_OPCODE:
      {
        ...
        if (eval_size < min_eval_size) {
          fvmc_selector_postfix_dump(pf, 0, 0, NULL, NULL);
          bftc_error(__FILE__, __LINE__, 0,
    ...
    if (eval_size == eval_max_size) {
      ...
      if (eval_stack == _eval_stack)
        BFTC_MALLOC(eval_stack, eval_max_size, _Bool);
      else
        BFTC_REALLOC(eval_stack, eval_max_size, _Bool);
    }
...
void
fvmc_selector_postfix_dump(const fvmc_selector_postfix_t  *pf,
                          int                            n_groups,
                          int                            n_attributes,
                          const char                    *group_name[],
                          const int                      attribute[])
{
    ...
    case PF_INT:
      {
        ...
        if (type == PF_GROUP_ID) {
          if (val < 0)
            bftc_printf("    %d (non-existing group id)\n", val);
          else if (n_groups > 0)
            bftc_printf("    %d (group: \"%s\")\n", val, group_name[val]);
          else
            bftc_printf("    %d (group id)\n");
        }
        else if (type == PF_ATTRIBUTE_ID) {
          if (val < 0)
            bftc_printf("    %d (non-existing attribute id)\n", val);
          else if (n_attributes > 0)
            bftc_printf("    %d (attribute: %d)\n", val, attribute[val]);
          else
            bftc_printf("    %d (attribute id)\n");
        }
        else
          bftc_printf("    %d\n", val);
        i += _postfix_int_size;
  ...
  if (pf->n_missing_operands > 0) {
    bftc_printf("  Missing operands:         %d\n",
               pf->n_missing_operands);
