  ...
  for (i = start_id; i < end_id; i++) {
    ...
    for (j = 0; j < dest_dim; j++) { /* Loop on destination dimension */
      ...
      for (j = 0; j < vertex_list_size; j++) {
        ...
        switch(src_datatype) {
        case FVMC_FLOAT:
          v_f = ((const float **)src_data)[pl][src_id];
          break;
        case FVMC_DOUBLE:
          v_f = ((const double **)src_data)[pl][src_id];
          break;
        default:
          assert(0);
        }
      ...
      case FVMC_FLOAT:
        ((float *)dest_data)[i] = (float) interpolated_value;
        break;
      case FVMC_DOUBLE:
        ((double *)dest_data)[i] = (double) interpolated_value;
        break;
      default:
        assert(0);
      }
