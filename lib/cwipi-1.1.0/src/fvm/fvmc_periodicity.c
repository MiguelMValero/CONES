...
typedef struct {
  ...
  fvmc_periodicity_type_t  type; /* Transformation type */
...
const char  *fvmc_periodicity_type_name[] = {N_("null"),
                                            N_("translation"),
                                            N_("rotation"),
                                            N_("mixed")};
...
static int
_combine_transforms_check(const fvmc_periodicity_t  *this_periodicity,
                          int                       tr_id_0,
                          int                       tr_id_1,
                          _Bool                     abort_on_error)
{
  ...
  if (   tr0->type != FVMC_PERIODICITY_TRANSLATION
      || tr1->type != FVMC_PERIODICITY_TRANSLATION) {
    ...
    for (i = 0; i < 3; i++) {
      for (j = 0; j < 4; j++) {
        ...
        if (  FVMC_ABS(m0[i][j] - m1[i][j])
            > this_periodicity->equiv_tolerance) {

          if (abort_on_error) {
            int ext0 = FVMC_ABS(tr0->external_num);
            int ext1 = FVMC_ABS(tr1->external_num);
            if (ext1 != 0) {
...
static void
_component_equiv_ids(fvmc_periodicity_t  *this_periodicity,
                     int                 tr_id_0,
                     int                 tr_id_1,
                     int                 comp_id[3])
{
...
static void
_combine_transforms(fvmc_periodicity_t  *this_periodicity,
                    int                 tr_id_0,
                    int                 tr_id_1,
                    int                 tr_id)
{
  ...
  if (tr0->type == tr1->type)
    ...
    tr->type = FVMC_PERIODICITY_MIXED;
                ...
                int            id)
{
  ...
  bftc_printf("\n"
             ...
             id,
             fvmc_periodicity_type_name[tr->type],
             tr->external_num,
             tr->reverse_id,
             tr->parent_ids[0], tr->parent_ids[1],
             tr->equiv_id);

  bftc_printf("  Matrix:              %12.5g %12.5g %12.5g %12.5g\n"
...
{
  int i;
  fvmc_periodicity_t  *period = NULL;

  BFTC_MALLOC(period, 1, fvmc_periodicity_t);
...
fvmc_periodicity_destroy(fvmc_periodicity_t  *this_periodicity)
{
  ...
  for (i = 0; i < this_periodicity->n_transforms; i++)
    BFTC_FREE(this_periodicity->transform[i]);

  BFTC_FREE(this_periodicity->transform);

  BFTC_FREE(this_periodicity);
                              ...
                              double                   matrix[3][4])
{
  ...
  BFTC_REALLOC(this_periodicity->transform,
  ...
  for (sub_transf = 0; sub_transf < 2; sub_transf++) {
    ...
    BFTC_MALLOC(transform, 1, _transform_t);
    ...
    for (i = 0; i < this_periodicity->n_transforms - 1; i++) {
      ...
      for (j = 0; j < 3; j++) {
        for (k = 0; k < 4; k++) {
          if (  FVMC_ABS(transform->m[j][k] - tr->m[j][k])
              > this_periodicity->equiv_tolerance)
            is_equiv = false;
        }
      }
                                ...
                                const double        translation[3])
{
  ...
  return fvmc_periodicity_add_by_matrix(this_periodicity,
                                       external_num,
    ...
    for (i = 0; i < n_level_0; i++) {
      ...
      for (j = j_start; j < j_end; j++) {
        ...
        if (retval == 0) {
          ...
          BFTC_MALLOC(this_periodicity->transform[tr_count], 1, _transform_t);
...
fvmc_periodicity_dump(const fvmc_periodicity_t  *this_periodicity)
{
  ...
  bftc_printf("Number of transforms  %d\n"
             "Number of levels  %d\n"
