...
static _Bool _bftc_timer_initialized = false;

/* Wall-clock time */

#if defined (HAVE_GETTIMEOFDAY)
static struct timeval  _bftc_timer_wtime_tv_start;
#else
static time_t _bftc_timer_wtime_start;
#endif
...
#if defined (HAVE_GETRUSAGE)
#elif defined(_POSIX_SOURCE)
static time_t _bftc_timer_unit = 0;
#else
static clock_t _bftc_timer_clock_start;
#endif
...
static void
_bftc_timer_initialize(void)
{
#if defined (HAVE_GETTIMEOFDAY)
  (void)gettimeofday(&_bftc_timer_wtime_tv_start, NULL);
#else
  (void)time(&_bftc_timer_wtime_start);
#endif

#if defined (HAVE_GETRUSAGE)
#elif defined(_POSIX_SOURCE)
  _bftc_timer_unit = (double)sysconf(_SC_CLK_TCK);
#else
  _bftc_timer_clock_start = clock();
#endif

  _bftc_timer_initialized = true;
}
 ...
 *
 * \return elapsed time from first call of a function of the bftc_timer_...()
 *         series, or -1 if unable to compute.
 */

double
bftc_timer_wtime(void)
{
  double this_wtime = -1.;

  /* Ensure initialization */

  if (_bftc_timer_initialized == false)
    _bftc_timer_initialize();
...

 {
    ...
    if (gettimeofday(&wtime_tv_current, NULL) == 0) {
      ...
      if (_bftc_timer_wtime_tv_start.tv_usec > wtime_tv_current.tv_usec) {
        int nsec = (_bftc_timer_wtime_tv_start.tv_usec - wtime_tv_current.tv_usec)
                   / 1000000 + 1;
        wtime_tv_current.tv_usec += 1000000 * nsec;
        wtime_tv_current.tv_sec -= nsec;
      }
      if (  wtime_tv_current.tv_usec - _bftc_timer_wtime_tv_start.tv_usec
          > 1000000) {
        int nsec = (wtime_tv_current.tv_usec - _bftc_timer_wtime_tv_start.tv_usec)
                   / 1000000;
        wtime_tv_current.tv_usec -= 1000000 * nsec;
        wtime_tv_current.tv_sec += nsec;
      }
      ...
      this_wtime =   (  wtime_tv_current.tv_sec
                      - _bftc_timer_wtime_tv_start.tv_sec)
                   + (  wtime_tv_current.tv_usec
                      - _bftc_timer_wtime_tv_start.tv_usec) * 1.e-6 ;
...

 {
   ...
   if (time(&wtime_current) != (time_t)-1)
     this_wtime = difftime(wtime_current, _bftc_timer_wtime_start);
 }
 ...
 * method is available (see bftc_timer_cpu_time_method()), at least one of
 * the bftc_timer_...() functions (possibly this one) must be called
...
double
bftc_timer_cpu_time(void)
{
  double cpu_time = -1.;

  /* Ensure initialization */

  if (_bftc_timer_initialized == 0)
    _bftc_timer_initialize();
...

 {
    ...
    if (_bftc_timer_unit != -1 && times(&ptimer) != -1) {
  ...
  if (_bftc_timer_clock_start != -1) {
    ...
    if (clock_current != (clock_t)-1)
      cpu_time
        = ((double)(clock_current - _bftc_timer_clock_start)) / CLOCKS_PER_SEC;
...
void
bftc_timer_cpu_times(double *user_time,
                    double *system_time)

{
...

 {
    ...
    if (_bftc_timer_unit != -1 && times(&ptimer) != -1) {
      *user_time    = ((double)ptimer.tms_utime)  / _bftc_timer_unit;
      *system_time  = ((double)ptimer.tms_stime)  / _bftc_timer_unit;
    }
...
const char *
bftc_timer_cpu_time_method(void)
{
  if (_bftc_timer_initialized == 0)
    _bftc_timer_initialize();
