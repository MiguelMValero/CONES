...
int main(int argc, char *argv[])
{
    ...
    if (cwipiSwitch)
    {
        ...
        addControlParams(numberCwipiPhase, cwipiStep, runTime.deltaTValue(), runTime.value(), cwipiMembers, cwipiObs, cwipiParams);
    ...
    while (runTime.loop())
    {
        ...
        //========= Receiving back updated Velocity Field ==========
        if (cwipiSwitch && cwipiPhaseCheck == 1)
        {
            cwipiRecv(mesh, U, runTime, cwipiIteration);
            cwipiRecvParams(mesh, U, cwipiParams);
