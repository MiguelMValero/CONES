...
Then, it is required to compile the solver cwipiIcoFoam. Go to the folder "solvers/cwipiIcoFoam" and compile it by doing (change the path of the headers of "cwipiPstream" inside the "solver/cwipiIcoFoam/Make/options" file in advance):
```
wclean
wmake
```

Next, the EnKF has to be compiled. Go to the folder "tests/cavity_test" and run:
```
make allclean
make all
```

To finish with, run the "cavity_test" case with the EnKF. Go to "tests/cavity_test" folder and run the bash file by doing:
```
./initCwipiIco
```
